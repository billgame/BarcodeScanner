...
class MultiCastSend {
    ...
    void sendGetUserMsg() {
        ...
        try {
            b = ("find@" + MultiCastDemo.msg).getBytes();
    ...
    void returnUserMsg(String ip) {
        ...
        try {
            b = ("retn@" + MultiCastDemo.msg).getBytes();
    ...
    void offLine() {
        ...
        DatagramPacket packet;
        try {
            b = ("offl@" + MultiCastDemo.msg).getBytes();
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    String orignal = new String(inPacket.getData(), 0, inPacket.getLength());
                    System.out.println("收到一个广播:"+orignal);
                    message = orignal.split("@");  //获取信息，并切割头部，判断是何种信息（find--上线，retn--回答，offl--下线）
                    
                    if (message[1].equals(MultiCastDemo.ip)) {
                        continue;  //忽略自身
                    }
